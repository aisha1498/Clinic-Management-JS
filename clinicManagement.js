const prompt = require("prompt-sync")();//taking input from user
const validateContact = require("./clinicRegex.js");
const fs = require('fs');
const { exit } = require("process");
function readfile() 
{
  let raw = fs.readFileSync('./add1.json','utf8');//enable user input at runtime
  let india= JSON.parse(raw);//convert text into js object
  console.log(india);
  console.log("raw",raw);
  return india;
  
}
function write(india){
  let data = JSON.stringify(india);//converts js object or value to json string
  console.log("data",data);
 return fs.writeFileSync('./add1.json', data);//creates a new file
...
function addContact() {
  ...
  try { //error handling
    ...
    myjson.push(personInfo);// adding new items at the end of the array
    console.log("personInfo",personInfo);
    //console.log("after push",myjson);
    write(myjson);//write string of text to a document.
    console.log(myjson);
  } catch (invalid) {// used to throw exception
...
function editContact() {
  ...
  let myjson =readfile();//holds the name of file to read the entire path if stored at other location
